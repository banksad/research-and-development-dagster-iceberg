...
CELL_NUMBER_MAPPED_RESPONSES_ASSET_KEY = dagster.AssetKey(["intermediate", "cell_number_mapped_responses"])
...
def _run_config() -> dict:
    ...
    return {
        "ops": {
            ...
            "ultfoc_mapper": {
                "config": {"ultfoc_mapper_csv_path": str(scenario / "ultfoc_mapper.csv")}
            },
            "cell_number_mapper": {
                "config": {
                    "cell_number_mapper_csv_path": str(scenario / "cell_number_mapper.csv")
                }
            },
        }
    }
...
def _build_defs(resource: TableStoreResource):
    ...
    return dagster.Definitions(
        assets=[
            ...
            getattr(mapping_mod, "cell_number_mapped_responses"),
        ],
        asset_checks=[
            ...
            getattr(mapping_checks_mod, "cell_number_mapped_responses_table_exists"),
            getattr(mapping_checks_mod, "cell_number_mapped_responses_non_empty"),
            getattr(mapping_checks_mod, "cell_number_mapped_responses_required_columns"),
            getattr(mapping_checks_mod, "cell_number_mapped_responses_unique_grain"),
            getattr(mapping_checks_mod, "cell_number_mapped_responses_mapping_columns_present"),
...
def test_staging_to_cell_number_mapping_chain_materialises_tables_and_runs_checks(tmp_path) -> None:
    scenario = scenario_path(SCENARIO_ID)
    resource = TableStoreResource(
        catalog_name="staging-to-cell-number-mapping-chain-smoke",
    ...
    actual_cell_number_mapped = store.read_table_as_dataframe(refs.INTERMEDIATE_CELL_NUMBER_MAPPED_RESPONSES)
    expected_cell_number_mapped = pd.read_csv(scenario / "expected_cell_number_mapped_responses.csv")
    assert_frame_equal_sorted(
        actual_cell_number_mapped,
        expected_cell_number_mapped,
        sort_by=["reference", "instance", "survey_type", "survey_year"],
    )
    ...
    ultfoc_mapper_checks_result = defs.get_asset_checks_def(ULTFOC_MAPPER_ASSET_KEY).execute_in_process()
    assert ultfoc_mapper_checks_result.success
    ...
    cell_number_mapper_checks_result = defs.get_asset_checks_def(CELL_NUMBER_MAPPER_ASSET_KEY).execute_in_process()
    assert cell_number_mapper_checks_result.success

    cell_number_mapped_checks_result = defs.get_asset_checks_def(
        CELL_NUMBER_MAPPED_RESPONSES_ASSET_KEY
    ).execute_in_process()
    assert cell_number_mapped_checks_result.success
