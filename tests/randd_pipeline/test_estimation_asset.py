...
def test_estimation_asset_key_and_config_validation() -> None:
    ...
    with pytest.raises(Exception): mod.MinimalEstimationWeightsConfig(cell_column=" ")
    with pytest.raises(Exception): mod.MinimalEstimationWeightsConfig(clear_statuses=[])
...
def test_estimated_responses_run_config_works(tmp_path) -> None:
    ...
    mod = importlib.import_module("src.randd_pipeline.assets.estimation")
    defs = dagster.Definitions(assets=[mod.estimated_responses], resources={"table_store": resource})
    result = defs.get_implicit_global_asset_job_def().execute_in_process(run_config={"ops": {"estimated_responses": {"config": {"clear_statuses": ["Clear"]}}}})
