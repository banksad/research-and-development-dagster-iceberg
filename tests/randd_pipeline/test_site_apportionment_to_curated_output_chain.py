...
def test_site_to_curated_chain_materialises_once_and_checks_pass(tmp_path):
    ...
    store.create_table_from_dataframe(
        ...
        load_scenario_csv("site_apportionment_to_curated_output_minimal", "input_site_apportioned_responses.csv"),
        overwrite=False,
    )

    defs = dagster.Definitions(
        assets=[assets_mod.curated_rnd_statistics],
        resources={"table_store": resource},
    )
    run_result = defs.get_implicit_global_asset_job_def().execute_in_process()
    assert run_result.success

    for fn in [
        checks_mod.curated_rnd_statistics_table_exists,
        checks_mod.curated_rnd_statistics_non_empty,
        checks_mod.curated_rnd_statistics_required_columns,
        ...
        checks_mod.curated_rnd_statistics_provenance_columns_present,
        checks_mod.curated_rnd_statistics_reconciles_to_site_input,
    ]:
        assert fn(resource).passed

    assert_frame_equal_sorted(
        store.read_table_as_dataframe(refs.CURATED_RND_STATISTICS),
        load_scenario_csv("site_apportionment_to_curated_output_minimal", "expected_curated_rnd_statistics.csv"),
