...
FOREIGN_SCENARIO_ID = "mapping_cell_number_minimal"
...
def _build_defs(resource):
    ...
    return dagster.Definitions(
        ...
        asset_checks=[
            ...
            getattr(checks_mod, "mapped_responses_cell_number_mapping_complete"),
...
def test_mapped_checks_pass_after_materialisation(tmp_path):
    ...
    resource.get_table_store().create_table_from_dataframe(refs.REF_CELL_NUMBER_MAPPER, pd.read_csv(scenario / "cell_number_mapper.csv"))
    defs = _build_defs(resource)
    assert defs.get_implicit_global_asset_job_def().execute_in_process().success
    result = defs.get_asset_checks_def(MAPPED_ASSET_KEY).execute_in_process()
...
def test_mapped_checks_fail_for_missing_cell_number_metadata_when_cellno_non_null(tmp_path):
    scenario = scenario_path(FOREIGN_SCENARIO_ID)
    df = pd.read_csv(scenario / "expected_cell_number_mapped_responses.csv")
    df.loc[0, "cellno"] = 1
    df.loc[0, "uni_count"] = "   "
    resource = TableStoreResource(catalog_name="mapped-meta", catalog_type="local_sql", warehouse=str(tmp_path / "warehouse"))
    resource.get_table_store().create_table_from_dataframe(refs.INTERMEDIATE_MAPPED_RESPONSES, df)
    res = _build_defs(resource).get_asset_checks_def(MAPPED_ASSET_KEY).execute_in_process(raise_on_error=False)
    assert not res.success
    assert any("non-null cellno" in m for m in _messages(res))
