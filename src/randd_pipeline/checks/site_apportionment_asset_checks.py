...
try:
    ...
    def site_apportioned_responses_non_empty(table_store: TableStoreResource):
        df, m = read_table_for_check(table_store, refs.INTERMEDIATE_SITE_APPORTIONED_RESPONSES, check_label="non-empty check")
        if m:
            return m
    ...
    def site_apportioned_responses_required_columns(table_store: TableStoreResource):
        df, m = read_table_for_check(table_store, refs.INTERMEDIATE_SITE_APPORTIONED_RESPONSES, check_label="required columns")
        if m:
            return m
    ...
    def site_apportioned_responses_unique_site_grain(table_store: TableStoreResource):
        df, m = read_table_for_check(table_store, refs.INTERMEDIATE_SITE_APPORTIONED_RESPONSES, check_label="unique site grain")
        if m:
            return m
    ...
    def site_apportioned_responses_site_identifier_populated(table_store: TableStoreResource):
        df, m = read_table_for_check(table_store, refs.INTERMEDIATE_SITE_APPORTIONED_RESPONSES, check_label="site id population")
        if m:
            return m
    ...
    def site_apportioned_responses_site_proportion_populated(table_store: TableStoreResource):
        df, m = read_table_for_check(table_store, refs.INTERMEDIATE_SITE_APPORTIONED_RESPONSES, check_label="site proportion population")
        if m:
            return m
    ...
    def site_apportioned_responses_apportioned_values_non_negative(table_store: TableStoreResource):
        df, m = read_table_for_check(table_store, refs.INTERMEDIATE_SITE_APPORTIONED_RESPONSES, check_label="apportioned values")
        if m:
            return m
    ...
    def site_apportionment_factors_required_columns(table_store: TableStoreResource):
        df, m = read_table_for_check(table_store, refs.REF_SITE_APPORTIONMENT_FACTORS, check_label="required columns")
        if m:
            return m
    ...
    def site_apportionment_factors_unique_site_grain(table_store: TableStoreResource):
        df, m = read_table_for_check(table_store, refs.REF_SITE_APPORTIONMENT_FACTORS, check_label="unique site grain")
        if m:
            return m
    ...
    def site_apportionment_factors_site_identifier_populated(table_store: TableStoreResource):
        df, m = read_table_for_check(table_store, refs.REF_SITE_APPORTIONMENT_FACTORS, check_label="site id population")
        if m:
            return m
        p, d = check_site_identifier_populated(df)
        return AssetCheckResult(passed=p, description=d)
    ...
    def site_apportionment_factors_proportions_sum_to_one(table_store: TableStoreResource):
        df, m = read_table_for_check(table_store, refs.REF_SITE_APPORTIONMENT_FACTORS, check_label="proportion sums")
        if m:
            return m
